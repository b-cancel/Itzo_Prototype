...
public class Enemy3FiniteStateManager : MonoBehaviour
{
        ...
        if (wanderState)
        {
            if (!closetScriptResult)
            {
                ...
                {//generate a random number to determine if the player is detected by enemy
                    ...
                    {//switching to the chaseState if the two random numbers equal, essetially acts as a probability generator,
